...
uint8_t g_adc3_dma_sta = 0;              /* DMA传输状态标志, 0,未完成; 1, 已完成 */
...
void HAL_ADC_MspInit(ADC_HandleTypeDef *hadc)
{
    ...
    /* AD采集引脚模式设置,模拟输入 */
    GPIO_Initure.Pin = ADC3_CHY_GPIO_PIN;        
    GPIO_Initure.Mode = GPIO_MODE_ANALOG; 
    GPIO_Initure.Pull = GPIO_PULLUP;      
    HAL_GPIO_Init(ADC3_CHY_GPIO_PORT, &GPIO_Initure);
}
 ...
 * @retval      无
 */
void adc3_dma_init(uint32_t mar)
{
    ...
    /* 配置对应ADC 端口 */
    g_adc3_handle.DMA_Handle = &g_dma_adc_handle;
    adc3_init();
    /* 
        需要在配置的时候开，但这里为了保证不变更之前的代码，
        另加一行设置g_adc3_handle.Init.ContinuousConvMode = ENABLE;
        配置ADC连续转换, DMA传输ADC数据
     */
    SET_BIT(g_adc3_handle.Instance->CR2, ADC_CR2_CONT);      /* CONT = 1, 连续转换模式 */

    /* 配置对应ADC通道 */
    adc3_channel_set(&g_adc3_handle , ADC3_CHY, ADC_REGULAR_RANK_1, ADC_SAMPLETIME_239CYCLES_5);    /* 设置通道，序列和采样时间 */

    HAL_NVIC_SetPriority(ADC_ADCX_DMACx_IRQn, 3, 3);
    HAL_NVIC_EnableIRQ(ADC_ADCX_DMACx_IRQn);
    HAL_ADC_Start_DMA(&g_adc3_handle,&mar,sizeof(uint16_t));
...
{
    __HAL_ADC_DISABLE(&g_adc3_handle);               /* 先关闭ADC */
    ...
    __HAL_ADC_ENABLE(&g_adc3_handle);                /* 重新启动ADC */
...
void adc3_nch_dma_init(uint32_t mar)
{
    ...
    g_adc3_handle.Instance = ADC_ADCX;
    g_adc3_handle.Init.DataAlign = ADC_DATAALIGN_RIGHT;       /* 右对齐 */
    g_adc3_handle.Init.ScanConvMode = ENABLE;                 /* 扫描模式 */
    g_adc3_handle.Init.ContinuousConvMode = ENABLE;           /* 连续转换 */
    g_adc3_handle.Init.NbrOfConversion = 6;                   /* 6个转换在规则序列中 也就是只转换规则序列1 */
    g_adc3_handle.Init.DiscontinuousConvMode = DISABLE;       /* 禁止不连续采样模式 */
    g_adc3_handle.Init.NbrOfDiscConversion = 1;               /* 连续采样通道数为0 */
    g_adc3_handle.Init.ExternalTrigConv = ADC_SOFTWARE_START; /* 软件触发 */
    HAL_ADC_Init(&g_adc3_handle);                             /* 初始化 */
    ...
    */
    ...
    /* 配置对应ADC通道 */
    adc3_channel_set(&g_adc3_handle , ADC_CHANNEL_0, ADC_REGULAR_RANK_1, ADC_SAMPLETIME_239CYCLES_5);    /* 设置ADCX对通道0(对应PA0脚)的采样时间为239.5个时钟周期 */
    adc3_channel_set(&g_adc3_handle , ADC_CHANNEL_1, ADC_REGULAR_RANK_2, ADC_SAMPLETIME_239CYCLES_5);    /* 设置ADCX对通道0(对应PA1脚)的采样时间为239.5个时钟周期 */
    adc3_channel_set(&g_adc3_handle , ADC_CHANNEL_2, ADC_REGULAR_RANK_3, ADC_SAMPLETIME_239CYCLES_5);    /* 设置ADCX对通道0(对应PA2脚)的采样时间为239.5个时钟周期 */
    adc3_channel_set(&g_adc3_handle , ADC_CHANNEL_3, ADC_REGULAR_RANK_4, ADC_SAMPLETIME_239CYCLES_5);    /* 设置ADCX对通道0(对应PA3脚)的采样时间为239.5个时钟周期 */
    adc3_channel_set(&g_adc3_handle , ADC_CHANNEL_4, ADC_REGULAR_RANK_5, ADC_SAMPLETIME_239CYCLES_5);    /* 设置ADCX对通道0(对应PA4脚)的采样时间为239.5个时钟周期 */
    adc3_channel_set(&g_adc3_handle , ADC_CHANNEL_5, ADC_REGULAR_RANK_6, ADC_SAMPLETIME_239CYCLES_5);    /* 设置ADCX对通道0(对应PA5脚)的采样时间为239.5个时钟周期 */
